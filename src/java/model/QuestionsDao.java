...
public class QuestionsDao {
	...
	public static boolean insertQuestion(Questions q) {
	    ...
	    pst.setString(6, q.getAnswer());
            pst.setString(7,String.valueOf(q.getMarks()));
		...
		try
		{
		   ...
		   while(rs.next())
		   {
			  ...
			  s.setD(rs.getString(5));
                          s.setMarks(Integer.parseInt(rs.getString(7)));
			   samp.add(s);
		   }
		}
		catch(Exception e2)
	...
	public static int doUpdateNowRecord(String OriginalQuestion,String newQuestion,String opta,String optb,String optc,String optd,String ans,String marks)
	{
		int status = 0;
		try
		{
		...
		Connection con = Provider.getOracleConnection();
		String sql = "update questiontable set question=?,a=?,b=?,c=?,d=?,answer=?,marks=? where question=?";
		PreparedStatement pst = con.prepareStatement(sql);
		...
		pst.setString(8, OriginalQuestion);
